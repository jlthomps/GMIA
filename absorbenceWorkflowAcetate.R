...
library(GSqwsr)
dataMerge <- dataMerge[which(!is.na(dataMerge$Acetate)),]
...
dataMerge$Acetate[which(dataMerge$rAcetate=="<")] <- .5*dataMerge$Acetate[which(dataMerge$rAcetate=="<")]
dataMerge$remark <- ""
...
data_sub_cens <- importQW(data_sub,keep=keepAll,"Acetate","remark","",0.0000002,"User","kg","Unk","","00310","Acetatecens")
siteName <- "GMIAAcetate"
siteNo <- '040871475'
...
investigateResponse <- "Acetatecens"
