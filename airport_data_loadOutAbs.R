...
load("C:/Users/jlthomps/Desktop/git/GMIA/testAbs.RData")
load("C:/Users/jlthomps/Desktop/git/GMIA/dataMerge2.RData")

data_sub <- testAbs[which(substr(testAbs$ProjectID,1,3)=="OUT"),]
...
data_sub_cens <- importQW(data_sub,c("Qmax","Eduration","snow_depth","GHCNDsnowDepth","mean_temp","max_temp","min_temp","prcp_sum","OUTkgGlycol","decYear","sinDY","cosDY"),"BODload","BODrmk","","RpLev","User","tons","Unk","","00310","BODLoading")
...
data_sub_cens <- importQW(data_sub,c("Qmax","Eduration","snow_depth","GHCNDsnowDepth","mean_temp","max_temp","min_temp","prcp_sum","OUTkgGlycol","decYear","sinDY","cosDY"),"EGPGload","EGPGrmk","","RpLev","User","tons","Unk","","91080","EGPGLoading")
siteName <- "OutfallAllFinal"
siteNo <- '040871475'
siteINFO <-  getSiteFileData(siteNo, interactive=FALSE)
...
pdf(fileName <- paste(pathToSave,"/","OutfallAllFinal","Residuals.pdf",sep=""))
...
lines(lowess(resids_BOD$data_sub.bpdate,resids_BOD$modelReturn.RESID,f=.4),col="blue")
...
pdf(fileName <- paste(pathToSave,"/","OutAllFinal","ResidualsBoxPlot.pdf",sep=""))
boxplot(residsdf,ylab="Model residual",main="Residuals for Outfall split into groups at 10/01/02 and 10/01/05",col=c("red","blue","green","red","blue","green","red","blue","green"),las=2,at=c(1,2,3,5,6,7,9,10,11),par(mar=c(12,5,4,2)+0.1))
dev.off()
