...
data_subPre <- data_sub[which(data_sub$bpdate<strftime("2002-10-01","%Y-%m-%d")),]
data_subPost <- data_sub[which(data_sub$bpdate>=strftime("2002-10-01","%Y-%m-%d")),]


####all
data_sub_cens <- importQW(data_sub,c("Qmax","Eduration","mean_temp","max_temp","min_temp","prcp_sum","OUTkgGlycol","decYear","sinDY","cosDY"),"CODload","CODrmk","",0.005,"User","tons","Unk","","00310","CODLoading")
siteName <- "OutfallAll"
siteNo <- '040871475'
...
par(mfrow=c(2,2))
plot(resids_56$data_sub.bpdate,resids_56$modelReturn.RESID,xlab="Datetime",ylab="COD Model Residuals",col="red",type="p",main=paste(siteName,"residuals .1",sep=" "))
lines(lowess(resids_56$data_sub.bpdate,resids_56$modelReturn.RESID,f=0.1),col="blue")
plot(resids_56$data_sub.bpdate,resids_56$modelReturn.RESID,xlab="Datetime",ylab="COD Model Residuals",col="red",type="p",main=paste(siteName,"residuals .2",sep=" "))
lines(lowess(resids_56$data_sub.bpdate,resids_56$modelReturn.RESID,f=0.2),col="blue")
plot(resids_56$data_sub.bpdate,resids_56$modelReturn.RESID,xlab="Datetime",ylab="COD Model Residuals",col="red",type="p",main=paste(siteName,"residuals .3",sep=" "))
lines(lowess(resids_56$data_sub.bpdate,resids_56$modelReturn.RESID,f=0.3),col="blue")
plot(resids_56$data_sub.bpdate,resids_56$modelReturn.RESID,xlab="Datetime",ylab="COD Model Residuals",col="red",type="p",main=paste(siteName,"residuals .4",sep=" "))
